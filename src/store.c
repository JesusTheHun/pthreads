...
static pthreads_storage* pthreads_store_create(zval *unstore, zend_bool complex){					
	...
	switch((storage->type = Z_TYPE_P(unstore))){
		...
		case IS_ARRAY: {
            ...
            pthreads_copy_hashtable(Z_ARRVAL_P(unstore), storage->data);
...
/* {{{ */
static HashTable* pthreads_copy_hashtable(HashTable *source, void *target) {
	uint32_t idx;
	...
	if (source->nNumUsed == 0) {
		...
		if (target->nNumUsed == target->nNumOfElements) {
			apc_array_dup_packed_elements(source, target, 0);
		} else {
			apc_array_dup_packed_elements(source, target, 1);
		}
		...
		if (target->u.flags & HASH_FLAG_STATIC_KEYS) {
			if (source->nNumUsed == source->nNumOfElements) {
				idx = apc_array_dup_elements(source, target, 1, 0);
			} else {
				idx = apc_array_dup_elements(source, target, 1, 1);
			}
		} else {
			if (source->nNumUsed == source->nNumOfElements) {
				idx = apc_array_dup_elements(source, target, 0, 0);
			} else {
				idx = apc_array_dup_elements(source, target, 0, 1);
			}
		}
		target->nNumUsed = idx;
...
/* {{{ */
static zend_always_inline void apc_array_dup_packed_elements(HashTable *source, HashTable *target, int with_holes)
{
	...
	do {
		if (!apc_array_dup_element(source, target, 0, p, q, 1, 1, with_holes)) {
			if (with_holes) {
				ZVAL_UNDEF(&q->val);
			}
		}
...
/* {{{ */
static zend_always_inline uint32_t apc_array_dup_elements(HashTable *source, HashTable *target, int static_keys, int with_holes)
{
    uint32_t idx = 0;
	Bucket *p = source->arData;
	Bucket *q = target->arData;
	...
	do {
		if (!apc_array_dup_element(source, target, idx, p, q, 0, static_keys, with_holes)) {
			uint32_t target_idx = idx;

			idx++; p++;
			while (p != end) {
				if (apc_array_dup_element(source, target, target_idx, p, q, 0, static_keys, with_holes)) {
...
/* {{{ */
static zend_always_inline int apc_array_dup_element(HashTable *source, HashTable *target, uint32_t idx, Bucket *p, Bucket *q, int packed, int static_keys, int with_holes)
{
	zval *data = &p->val;
...
static int pthreads_store_convert(pthreads_storage *storage, zval *pzval){
	...
	switch(storage->type) {
		...
		case IS_OBJECT:
			result = pthreads_store_obj_tozval(pzval, (char*) storage->data, storage->length);
			if (result == FAILURE) {
			    ZVAL_NULL(pzval);
			}
		break;
		...
		case IS_ARRAY:
            result = pthreads_store_arr_tozval(pzval, (char*) storage->data, storage->length);
            if (result == FAILURE) {
                ZVAL_NULL(pzval);
            }
		break;
...
/* {{{ */
static int pthreads_store_obj_tozval(zval *pzval, char *pstring, size_t slength) {
	int result = SUCCESS;
...
/* {{{ */
static int pthreads_store_arr_tozval(zval *pzval, char *pstring, size_t slength) {
	int result = SUCCESS;
